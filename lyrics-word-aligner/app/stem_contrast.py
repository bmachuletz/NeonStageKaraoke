...
def refine_final_releases_with_stem_contrast(
    ...
    Only final words are shortened.  Onsets, internal word boundaries and
    genuinely held notes are outside this pass, and no release is ever
    extended.  The small padding retains the audible consonant/release tail
    expected by the karaoke renderer.
    """
    ...
    for line_index, line in enumerate(lines):
        ...
        if (getattr(line, "manual_adjusted", False)
                or word.get("editor_word_manual_adjusted")):
            continue
